...
public class AccessDAO {
  ...
  /**
   * GetAllModuleCodes — every active ModuleCode in the table, regardless of
   * role. Role-independent on purpose: ModuleReconciler (BKL-24) needs the
   * full seeded set to diff against the ModuleCode enum and the registered
   * views. GetModulesForRole cannot serve this — it only ever returns what one
   * role was granted, so a module no role holds is invisible to it.
   */
  public List<String> GetAllModuleCodes() {
    List<String> codes = new ArrayList<>();
    String sql =
      "SELECT m.ModuleCode FROM Module m " +
      "WHERE m.Status = 1 ORDER BY m.ModuleCode";
    ...
    try (
      ...
      while (rs.next()) {
        codes.add(rs.getString("ModuleCode"));
      }
    } catch (SQLException e) {
      System.err.println("AccessDAO.GetAllModuleCodes: " + e.getMessage());
    }
    return codes;
  }
  ...
  /**
   * GetPermissionMatrix — the full role x module x permission grant matrix
   * for one role, as ModuleCode -> granted PermissionCodes, in a single query.
   *
   * BKL-25: replaces the pattern of one GetPermissionCodes(roleId, moduleCode)
   * call per gated registration — each of which opened its own connection, on
   * every panel mount. Injector.CreateShell() calls this exactly once, right
   * after login, and caches the result on Session; every gated registration
   * then reads Session.GetPermissions(code) with no further DB access.
   */
  public Map<String, List<String>> GetPermissionMatrix(int roleId) {
    Map<String, List<String>> matrix = new HashMap<>();
    String sql =
      "SELECT m.ModuleCode, p.PermissionCode " +
      "FROM Role_Permission rp " +
      "JOIN Module m     ON m.ModuleID     = rp.ModuleID " +
      "JOIN Permission p ON p.PermissionID = rp.PermissionID " +
      "WHERE rp.RoleID = ? AND rp.Status = 1 AND m.Status = 1 AND p.Status = 1 " +
      "ORDER BY m.ModuleCode, p.PermissionCode";
    ...
    try (
      ...
      try (ResultSet rs = ps.executeQuery()) {
        while (rs.next()) {
          matrix
            .computeIfAbsent(rs.getString("ModuleCode"), k -> new ArrayList<>())
            .add(rs.getString("PermissionCode"));
        }
      }
    } catch (SQLException e) {
      System.err.println("AccessDAO.GetPermissionMatrix: " + e.getMessage());
    }
    return matrix;
  }
}
